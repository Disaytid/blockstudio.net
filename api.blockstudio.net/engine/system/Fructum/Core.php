<?php
	...
	class Core
	{
		...
		protected static function hooks_autoloader($class)
		{
			...
			if(!file_exists(self::root() . self::SEPARATOR . self::HOOKS . self::SEPARATOR . $class . self::EXT)) { 
				return; 
			}
		...
		protected static function system_autoloader($class)
		{
			if(class_exists($class, false)) { return; }
			$class = str_replace('\\', '/', $class);
			if(!file_exists(self::root() . self::SEPARATOR . self::SYS . self::SEPARATOR . $class . self::EXT)) { 
				return; 
			}
